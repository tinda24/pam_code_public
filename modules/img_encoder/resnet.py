...
class ResnetEncoder(nn.Module):
    ...
    def __init__(
        ...
        if frozen:
            if in_channels != 3:
                raise Exception(
                    "[error] cannot frozen pretrained "
        ...
        self.projection_layer = SpatialProjection(output_shape[1:], output_dim)
    ...
    def forward(self, x, lang=None, return_intermediate=False):
        ...
        h = self.block_1(h)
        if lang is not None and self.language_fusion != "none":
            B, C, H, W = h.shape
...
class ResnetRouter(nn.Module):
    def __init__(self, input_shape,
        ...
        self.language_fusion = language_fusion
        self.device = device
        self.encoder = ResnetEncoder(input_shape, output_dim, pretrained, frozen, remove_layer_num, no_stride, language_dim, language_fusion).to(device)
        self.encoder = self.encoder.to(device)
        self.separate_encoders = False

    def forward(self, pixel, lang=None):
        assert lang is not None, "language is required for language fusion"
        return self.encoder(pixel, lang)
