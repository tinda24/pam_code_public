...
class ContinuousRectifiedFlow(DiffusionModel):
    ...
    def loss(
        ...
        loss = (self.model["diffusion"](xt, t, action_cond, addition_cond) - (x0 - x1)) ** 2
        valid_mask = valid_mask.unsqueeze(-1).repeat(1,1,x0.shape[-1])
...
class ImgHead(nn.Module):
    def __init__(self,
                ...
                device='cuda',
                past_img_pred_flag_for_mainagent=True
                ):
        super().__init__()
        self.past_img_pred_flag_for_mainagent = past_img_pred_flag_for_mainagent
        ...
        nn_diffusion = DiT1dWithACICrossAttention(
            stage='base',
            x_dim=z_length*z_dim,
            x_seq_len=self.max_his_cache,
            emb_dim=cond_dim,
            d_model=d_model,
            n_heads=num_heads,
            depth=num_layers,
        ...
        self.policy = ContinuousRectifiedFlow(
            ...
            nn_condition=None,
            x_max=torch.full((self.max_his_cache, z_length*z_dim), 1.0, device=self.device),
            x_min=torch.full((self.max_his_cache, z_length*z_dim), -1.0, device=self.device),
        ).to(self.device)

    def forward(self, img_cond, valid_mask=None, gt_img=None,init_noise=None):

        if gt_img is None:
            B = img_cond.shape[0]
            act, log = self.policy.sample(
                prior=torch.zeros((B, self.max_his_cache, self.z_dim*self.z_length), device=self.device),
                action_cond=img_cond,
                addition_cond=None,
                x1=init_noise,
                solver="euler",
        ...
        gt_img_flat = einops.rearrange(gt_img, 'b cache l d -> b cache (l d)')
