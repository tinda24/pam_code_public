...
class multi_factor_attn(nn.Module):
    def __init__(self, n_embd, n_head, dropout,use_gate_fusion = False):
        super().__init__()
        ...
        self.addition_K = nn.Linear(n_embd, n_embd)
        self.addition_V = nn.Linear(n_embd, n_embd)
    ...
    def forward(self, query, key, value, addition_key, addition_value, key_padding_mask = None, addition_key_padding_mask = None):
        B, T, C = query.size()
        _, T_kv1, _ = key.size()
        _, T_kv2, _ = addition_key.size()

        q = self.Q(query)
        k = self.K(key)
        v = self.V(value)
        addition_k = self.addition_K(addition_key)
        addition_v = self.addition_V(addition_value)

        q = q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2)

        k = k.view(B, T_kv1, self.n_head, C // self.n_head).transpose(1, 2)
        v = v.view(B, T_kv1, self.n_head, C // self.n_head).transpose(1, 2)
        addition_k = addition_k.view(B, T_kv2, self.n_head, C // self.n_head).transpose(1, 2)
        addition_v = addition_v.view(B, T_kv2, self.n_head, C // self.n_head).transpose(1, 2)

        att1 = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))

        att2 = (q @ addition_k.transpose(-2, -1)) * (1.0 / math.sqrt(addition_k.size(-1)))

        if key_padding_mask is not None:
            att1 = att1.masked_fill(key_padding_mask.unsqueeze(1), float("-inf"))
        if addition_key_padding_mask is not None:
            att2 = att2.masked_fill(addition_key_padding_mask.unsqueeze(1), float("-inf"))
...
class DiTParallelConditionControlNetBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_cross_attn:
            ...
            self.multi_attn = multi_factor_attn(
                hidden_size, n_heads, attn_dropout, use_gate_fusion
            )
        else:
            self.ca_norm, self.multi_attn = None, None
    ...
    def forward(
        ...
        addition_condition: Optional[torch.Tensor] = None,
        seq_condition_mask: Optional[torch.Tensor] = None,
        addition_condition_mask: Optional[torch.Tensor] = None,
        vaild_mask = None,
    ):
        ...
        if self._use_cross_attn:
            ...
            x = (
                ...
                + gate_ca
                * self.multi_attn(
                    h, seq_condition, seq_condition, addition_condition,addition_condition,key_padding_mask=seq_condition_mask,addition_key_padding_mask=addition_condition_mask
                )
            )
    ...
    def act_analysis(
        ...
        if self._use_cross_attn:
            ...
            y, attn_map = self.multi_attn(h, seq_condition, seq_condition, addition_condition,addition_condition,key_padding_mask=seq_condition_mask)
            x = x + gate_ca* y
...
class DiT1dWithACICrossAttention(BaseNNDiffusion):
    ...
    def __init__(
        ...
        self.stage = stage
        assert self.stage == 'post', "only supports post stage"
        ...
        if self.post_type == 'last' or self.post_type == 'first' or self.post_type == 'concat':
            self.blocks = nn.ModuleList(
                [
                    DiTBlock(
                        d_model, n_heads, attn_dropout, ffn_dropout, use_cross_attn, adaLN_on_cross_attn
                    )
                    for _ in range(depth+num_new_layers)
                ]
            )
        elif self.post_type == 'parallel':
            self.blocks = nn.ModuleList(
                [
                    DiTParallelConditionControlNetBlock(
                        d_model, n_heads, attn_dropout, ffn_dropout, use_cross_attn, adaLN_on_cross_attn, use_gate_fusion=False
                    )
                    for _ in range(depth+num_new_layers)
                ]
            )
        else:
            raise ValueError(f"Invalid post type: {self.post_type}")
    ...
    def forward(
        ...
        if self.post_type == 'concat':
            for i, block in enumerate(self.blocks):
                seq_condition = torch.cat([action_cond, addition_cond], dim=1)
                seq_condition_mask = None
                x_emb = block(x_emb, cond_emb, seq_condition, seq_condition_mask)

        elif self.post_type == 'parallel':
            for i, block in enumerate(self.blocks):
                x_emb = block(x_emb, cond_emb, seq_condition = action_cond, addition_condition = addition_cond)

        elif self.post_type == 'last':
            for i, block in enumerate(self.blocks):
                seq_condition = addition_cond if i == len(self.blocks)-1 else action_cond
                seq_condition_mask = None
                x_emb = block(x_emb, cond_emb, seq_condition, seq_condition_mask, vaild_mask = vaild_mask)

        elif self.post_type == 'first':
            for i, block in enumerate(self.blocks):
                seq_condition = addition_cond if i ==0 else action_cond
                seq_condition_mask = None
                x_emb = block(x_emb, cond_emb, seq_condition, seq_condition_mask, vaild_mask = vaild_mask)
