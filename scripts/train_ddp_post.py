...
class WorkspaceIL:
    def __init__(self, cfg, rank=0, world_size=1):
        ...
        self.rank = rank
        self.relative_rank = rank % world_size
        ...
        if self.cfg.stage == "post":

            base_cfg = OmegaConf.load(self.cfg.base_config_dir)
            self.cfg.suite = base_cfg['suite']

            base_cfg = OmegaConf.load(cfg.base_config_dir)
            for key in base_cfg['agent'].keys():
                if key in ['img_encoder','backbone','action_dim','proprio_dim','obs_hw','num_views','img_size',
                           'pixel_keys','proprio_key','lang_key','hidden_dim','num_queries','action_arrange_type',
                           'action_loss_coef','lang_encoder','use_proprio','use_language','history_len','num_act_tokens',
                           'num_img_tokens']:
                    self.cfg.agent[key] = base_cfg['agent'][key]
        ...
        self.logger = Logger(self.work_dir, use_tb=True, rank=rank)
    ...
    def load_train_message(self):
        path = Path(self.cfg.bc_train_message_dir)
        map_location = self.device

        ckpt = torch.load(str(path), weights_only=False, map_location=map_location)
        ...
        print(f"Loaded training state from {path}, step={step}, episode={episode}")
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            ...
            if metrics["img_loss"] == 0:
                loss = metrics["actor_loss"]
            else:
                loss = metrics["actor_loss"] + metrics["img_loss"]
            ...
            if log_every_step(self.global_step):
                ...
                with self.logger.log_and_dump_ctx(self.global_frame, ty="train") as log:
                    ...
                    if "img_loss" in metrics:
                        log("img_loss", metrics["img_loss"])
                    log("step", self.global_step)

            if fig_every_step(self.global_step) and self.global_step != 0:
                plot_losses(self.work_dir,self.rank)

            dist.barrier()
            if save_every_step(self.global_step) and self.rank == self.cfg.multi_gpu[0] and self.global_step!=0:
                self.save_snapshot()
...
def train_worker(rank, world_size, cfg):
    ...
    try:
        ...
        workspace = WorkspaceIL(cfg, rank=gpu_id, world_size=world_size)
        workspace.train()
